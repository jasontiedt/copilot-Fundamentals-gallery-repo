# Lab 9: Migrate Tcl → Python with Copilot

**Scenario:** Your team owns a **legacy Tcl** utility, `gallery_report.tcl`, that still runs in production but nobody wants to maintain it. You've been asked to **reimplement it in Python** — with **identical behavior**. This lab shows how Copilot makes a language migration safe and fast: understand the old code, **de-risk the ambiguous parts by asking the original intent**, plan the port, convert incrementally, and prove **behavior parity**.

This is the **Migration & Modernization** lab — an advanced, single-scenario lab (source: **Tcl**, target: **Python**), so there's no language picker.
...
git checkout -b USERNAME/lab-9-tcl-to-python
cd labs/starter/tcl
```

## ✅ Definition of Done
- [ ] A `gallery_report.py` with the **same CLI** and **byte-identical output** to the Tcl for `report`, `top`, `search`, and `filter`.
- [ ] The ambiguous `score` truncation is confirmed and preserved.
...
- [ ] Build a **custom agent** that specializes in Tcl→Python migration
...
```markdown
#file:gallery_report.tcl Explain what this Tcl program does end to end: its subcommands, the data format, and any Tcl idioms (dict, lsort -command, regsub) that won't map 1:1 to Python.
```
...
Create `.github/agents/tcl-to-python.agent.md`:
```markdown
---
name: Tcl to Python Migrator
description: Ports Tcl to behavior-identical Python, asking before guessing
tools: ['search/codebase', 'edit/editFiles', 'search']
---

# Tcl → Python Migrator

You port legacy **Tcl** to idiomatic **Python 3** with **identical observable behavior**.

Rules:
- **Preserve behavior exactly**, including integer truncation, sort stability, and output formatting/whitespace.
- Map idioms faithfully: `dict` → dict, `lsort -command` → `sorted(key=…)` / `functools.cmp_to_key`, `regsub` → `re.sub`, `expr {int(x)/100}` → `x // 100`.
- **Never invent behavior.** If a proc's intent is ambiguous, **stop and ask me** a specific question before porting it.
- Keep the **CLI and output byte-identical**. Add pytest tests that compare against the Tcl golden master.
- Work in small steps and explain each mapping decision briefly.
```

Create it via `/agents` → **New Agent** (or **Configure Chat** ⚙ → **Agents** tab), then select **Tcl to Python Migrator** from the mode picker to confirm it loads.
...
**2.** Open `gallery_report.tcl`, run `/annotate-tcl`, and **answer its questions**. Expect it to ask about the `score` weights and the `/100` truncation — tell it the truncation is intentional so the Python port matches.
...
```markdown
Create a migration plan to port gallery_report.tcl to a byte-identical gallery_report.py:
- inventory each proc and its Python target,
- an idiom mapping table (Tcl → Python),
- conversion order (leaf helpers first: load_photos, normalize, score, then top_by, search, format_report, then the CLI),
- a parity-test strategy against the golden-master output,
- risks (integer truncation, sort stability, output whitespace).
```
...
| Tcl | Python |
| --- | ------ |
| `set x 5` / `proc f {a b}` | `x = 5` / `def f(a, b):` |
| `dict create/get/set` | dict literal / `d[k]` |
| `lappend` / `lindex` / `llength` / `lrange` | `.append` / `lst[i]` / `len` / slice |
| `foreach x $xs {}` | `for x in xs:` |
| `lsort -command cmp` | `sorted(xs, key=…)` or `functools.cmp_to_key` |
| `split` / `join` | `str.split` / `"…".join` |
| `regexp` / `regsub -all` | `re.search` / `re.sub` |
| `expr {int($x)/100}` | `int(x) // 100` *(truncates!)* |
| `open`/`gets`/`close` | `with open(p) as f: for line in f:` |
| `switch` / `puts` / `format` | `if/elif` (or `match`) / `print` / f-strings |
...
**Your goal:** With the **Tcl to Python Migrator** agent selected and your plan in hand, port the program to `gallery_report.py` — **leaf helpers first**, checking parity as you go. Don't paste one giant prompt; drive it stage by stage.

**Done when:**
- [ ] `gallery_report.py` exists with the same subcommands and argument order.
- [ ] Each helper was ported in plan order, and you reviewed every edit.
- [ ] The `score` truncation uses `//` (per your Stage 3 answer).

<details><summary>💡 Stuck? Reveal a hint</summary>

Ask the migrator agent, one step at a time: "Port `load_photos` to Python matching the data format," then "now `score` (keep integer truncation)," etc. After each, run `python gallery_report.py photos.dat report` and compare.
...
**Your goal:** Make the Python output **identical** to the Tcl.

**Done when:**
- [ ] `python gallery_report.py photos.dat report` matches the golden master exactly.
- [ ] You have a **parity test** (pytest comparing outputs, or a committed `diff` check) for `report` and `top views 3`.
- [ ] Any mismatch was fixed with `/fix` (watch for `/` vs `//`, `%-22s` padding, tie order).

<details><summary>💡 Stuck? Reveal a hint</summary>

```bash
diff <(python gallery_report.py photos.dat report) expected_report.txt && echo PARITY_OK
```
Or ask Copilot to "write a pytest that runs both report commands and asserts equal output." If padding differs, match the `format "%-22s %5d pts  (%d views)"` widths.
...
1. **Commit** `gallery_report.py`, your parity test, the annotated Tcl, and the new agent + prompt files (generate the message with Copilot).
2. **Push:** `git push -u origin USERNAME/lab-9-tcl-to-python`
...
**🎉 Success:** A legacy Tcl tool reborn as behavior-identical Python — with reusable migration tooling.

## ✅ Completion Checklist
- [ ] Explained the Tcl with `/explain` and `#file`
- [ ] Built the `tcl-to-python.agent.md` migration agent
...
- [ ] Ported to `gallery_report.py` incrementally
