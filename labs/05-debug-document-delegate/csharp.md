# Lab 5 · C# Track — Debug, Document & Delegate
...
**1.** Re-enable the test you quarantined in Lab 3: remove the `Skip = "..."` from the `MostViewed` test's `[Fact]`/`[Theory]`.
...
**3.** Open [GalleryService.cs](../starter/csharp/GalleryService.cs), select `MostViewed`, and use **`/fix`** — describe the bug in your own words and let Copilot propose the correction.

**4.** Re-run `dotnet test ../Gallery.Tests` until everything, including the previously-skipped test, is green.

<details><summary>💡 Stuck? Reveal a hint</summary>

The sort direction is wrong — `MostViewed` should return the *highest* views first. Tell `/fix` exactly that.
...
**Done when:**
- [ ] The service methods have concise XML docs.
