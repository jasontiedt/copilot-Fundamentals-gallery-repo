# Lab 5 · Java Track — Debug, Document & Delegate

**Goal:** Fix the planted `mostViewed` bug guided by your Lab 3 test, document the service, and delegate a follow-up to the Copilot coding agent.
...
**1.** Re-enable the test you quarantined in Lab 3: remove the `@Disabled(...)` from the `mostViewed` test in `GalleryServiceTest`.
...
**3.** Open [GalleryService.java](../starter/java/src/main/java/com/example/gallery/GalleryService.java), select `mostViewed`, and use **`/fix`** — describe the bug in your own words and let Copilot propose the correction.

**4.** Re-run `mvn -q test` until everything, including the previously-disabled test, is green.

<details><summary>💡 Stuck? Reveal a hint</summary>

The comparator direction is wrong — `mostViewed` should return the *highest* views first. Tell `/fix` exactly that.

</details>

**✅ Checkpoint:** All tests, including the previously-disabled one, are green.
...
**Done when:**
- [ ] The service methods have concise Javadoc.
- [ ] You generated a 6–8 line "Using GalleryService" section for the module README.

<details><summary>💡 Stuck? Reveal a hint</summary>

`/doc` documents a selection. Then ask Copilot to "write a 6–8 line usage section showing filterPhotos, getPage, getPhotoDetail, and mostViewed."
...
- [ ] Re-enabled the test and fixed `mostViewed` with `/fix`
