# Lab 10 · Java Track — Local Code Review Agent
...
> **Recommended prerequisite:** [Lab 1](../01-personalize-and-standardize/java.md) (agent & skill basics), [Lab 3](../03-unit-testing/java.md) (JUnit 5 is set up), and [Lab 4](../04-reviewing-changes/java.md) (review fundamentals). Not required, but this lab builds directly on all three.
...
In **Agent mode**:
```markdown
In GalleryService.java, add a method `String exportPhotographerReport(String photographerName, String outputDir)` that filters photos by photographerName (reuse filterPhotos), writes a plain-text report (title, likes, views per photo) to a file at Paths.get(outputDir, photographerName + ".txt"), and returns the path written. Follow our java.instructions.md. Don't add tests yet — that's for a later stage.
```
Accept the change.
...
**✅ Checkpoint:** `GalleryService.java` has a new `exportPhotographerReport` method (no tests yet) that builds its output path directly from `photographerName`.
...
- [ ] Flags file paths built with `Paths.get`/`Files.write` from caller-supplied input without confirming the resolved path stays inside the intended directory (path traversal) as **Blocking**.
- [ ] Flags SQL built via string concatenation, unsafe deserialization (`ObjectInputStream` on untrusted data), hard-coded secrets, and missing input validation at a trust boundary as **Blocking**.
...
> Create a SKILL.md at `.github/skills/code-review-security/` for reviewing diffs for security issues: flag file paths built from caller-supplied input (Paths.get/Files.write) without confirming the resolved path stays inside the intended directory as Blocking, alongside string-concatenated SQL, unsafe deserialization, hard-coded secrets, and missing validation at trust boundaries; unredacted input in logs and broad catch blocks as Suggestion. Require file:line and a suggested fix for every finding. Add `name` and `description` frontmatter.
...
Create it via `/agents` → **New Agent** (or **Configure Chat** ⚙ → **Agents** tab), or add the file directly under `.github/agents/`. Give it `tools: ['search/codebase', 'search']` — deliberately no `editFiles`. In the body: "You are a senior Java reviewer running a pre-push gate on this developer's local, uncommitted changes. Never edit files. Check the diff against the `code-review-security` and `code-review-coverage` skills and any scoped instructions. Report Blocking / Suggestions / Nits with file:line and a fix. End with one verdict line: ✅ Ready to push, or 🚫 Not ready — N blocking item(s)."
...
```markdown
Harden exportPhotographerReport: reject a photographerName containing path separators or "..", then confirm the resolved absolute path stays inside outputDir (Path#normalize plus a containment check) before writing. Throw IllegalArgumentException for an invalid name.
```

**3. Fix the coverage finding** — add tests:
```markdown
Add tests to GalleryServiceTest for exportPhotographerReport: a normal photographer name writes a file under a @TempDir outputDir, an unknown photographer still writes an (empty) report, and a name containing "../" or a path separator throws IllegalArgumentException. Use @ParameterizedTest for the invalid names.
```
Run them:
```bash
mvn -q test
