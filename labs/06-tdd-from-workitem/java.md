# Lab 6 · Java Track — TDD from a Work Item
...
## ✅ Definition of Done
```bash
mvn -q test   # all green, including the new mostLiked tests
```
...
```markdown
Here is work item US-142 (paste ACs). List the JUnit 5 test cases I should write for a new `GalleryService.mostLiked(int limit)` method, including edge cases. Don't write code yet — just the test list.
```
...
**1.** Open [labs/starter/java/src/main/java/com/example/gallery/GalleryService.java](../starter/java/src/main/java/com/example/gallery/GalleryService.java) to see the `Photo` accessors and the constructor (it accepts a custom photo list — handy for tie tests).

**2.** In `src/test/java/com/example/gallery/GalleryServiceTest.java`, ask Copilot to write **only the tests** for `mostLiked` from your Stage 1 list — calling a method that doesn't exist yet.

**3.** Run them and confirm they **fail to compile / fail**:
```bash
mvn -q test -Dtest=GalleryServiceTest
```
...
In **Agent** mode:
```markdown
Implement `List<Photo> mostLiked(int limit)` in GalleryService.java to make the failing tests pass: sort by likes descending, break ties by id ascending (use a Comparator), return an empty list when limit <= 0. Follow our java.instructions.md. Don't change the tests.
```
Run `mvn -q test` until **green**.
...
**Your goal:** Improve the implementation **without changing behavior** — the tests are your safety net (e.g., a shared comparator, Javadoc, no duplication with `mostViewed`).

**Done when:**
- [ ] You made at least one readability/structure improvement.
- [ ] `mvn -q test` is still green after every change.

<details><summary>💡 Stuck? Reveal a hint</summary>

`mostViewed` and `mostLiked` are near-identical stream pipelines — ask Copilot to "extract a shared private helper parameterized by comparator, without changing behavior; keep tests green."
...
**Your goal:** Create `.github/prompts/workitem-to-tests.prompt.md` that turns any pasted work item into a JUnit 5 test list.
...
Create it via `/prompts` → **New Prompt** (or `/create-prompt`). Body: "Given a work item, list JUnit 5 cases mapping each acceptance criterion to tests, including edge cases. Output only the list."
