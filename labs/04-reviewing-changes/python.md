# Lab 4 · Python Track — Reviewing Changes
...
```markdown
In gallery/service.py, add a `count` property that returns the number of photos, with a one-line docstring. Follow our python.instructions.md.
```
Accept the change.

**✅ Checkpoint:** You have an unstaged change in `service.py`.
...
2. `.github/instructions/code-review.instructions.md` (scoped with `applyTo` for `**/*.py`)
...
Ask Copilot: "Draft a concise PR template and a `code-review.instructions.md` (applyTo `**/*.py`) that checks type hints, specific exceptions, no mutable default args, small pure functions, and pytest coverage." Then trim to your team's reality.
