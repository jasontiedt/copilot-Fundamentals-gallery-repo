# Lab 4 · C# Track — Reviewing Changes
...
```markdown
In GalleryService.cs, add a `Count` property that returns the number of photos, with a one-line XML doc. Follow our csharp.instructions.md.
```
Accept the change.

**✅ Checkpoint:** You have an unstaged change in `GalleryService.cs`.
...
2. `.github/instructions/code-review.instructions.md` (scoped with `applyTo` for `**/*.cs`)
...
Ask Copilot: "Draft a concise PR template and a `code-review.instructions.md` (applyTo `**/*.cs`) that checks nullable respected, no null collections, records for immutable data, async/await with CancellationToken, and xUnit coverage." Then trim to your team's reality.
