# Lab 4 · TypeScript / React Track — Reviewing Changes

**Goal:** Practice Copilot-assisted review end to end, from inline hints to a Pull Request review — using the gallery app.
...
```markdown
In src/components/gallery/GalleryGrid.tsx, add a small guard: if `limit` is less than 1, treat it as the default of 6. Add a one-line comment explaining why.
```
Accept the change.

**✅ Checkpoint:** You have an unstaged change in `GalleryGrid.tsx`.

---

## 🎯 Stage 2: Inline review (3 min)

1. **Select** the code you just changed.
...
2. `.github/instructions/code-review.instructions.md` (scoped with `applyTo` for `.ts`/`.tsx`)
...
Ask Copilot: "Draft a concise PR template and a `code-review.instructions.md` (applyTo `**/*.ts,**/*.tsx`) for a Next.js + TypeScript + Tailwind app that checks typed props, explicit loading/empty states, dark-mode variants, and test coverage." Then trim to your team's reality.
