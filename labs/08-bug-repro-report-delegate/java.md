# Lab 8 · Java Track — Bug Reproduction → Report → Delegate
...
**1.** Observe the defect: run the app and note that `mostViewed` lists the **least**-viewed photos first (open [GalleryService.java](../starter/java/src/main/java/com/example/gallery/GalleryService.java) — the comparator is ascending).

**2.** Ask Copilot (Ask/Edit mode) to write a **failing** test in `src/test/java/com/example/gallery/GalleryServiceTest.java`:
```markdown
Write a JUnit 5 test asserting mostViewed(1) returns the single HIGHEST-viewed photo. Don't fix the method — the test should fail.
```
...
2. Confirm it reverses the comparator and that **your repro test now passes** on its branch.
