# Lab 8 · Python Track — Bug Reproduction → Report → Delegate
...
**1.** Observe the defect directly:
```bash
python -c "from gallery.service import GalleryService; print([p.title for p in GalleryService().most_viewed(3)])"
```
You'll see the **least**-viewed photos — the ranking is backwards.

**2.** Ask Copilot (Ask/Edit mode) to write a **failing** test in `tests/test_service.py`:
```markdown
Write a pytest test asserting most_viewed(1) returns the single HIGHEST-viewed photo. Don't fix the method — the test should fail.
```
...
2. Confirm it sorts descending and that **your repro test now passes** on its branch.
