...
Reuse the frontmatter keys from `Plan.agent.md` (`name`, `description`, `tools`, optional `focusArea`). In the body: "You are a senior Java reviewer. Do not edit code. Report **Blocking / Suggestions / Nits** with file+line, checked against our instructions."
